...
class Index extends Controller
{
    ...
    public function login()
    {
        ...
        if (isset($data_json->phone) && $data_json->validate) {
            $checkRes = checkTel($data_json->phone);
            if ($checkRes) {
                $code = Cache::get($data_json->phone);
                if (!empty($code)) {
                    if ($data_json->validate == $code) {
                        $default_head = Config::get('DEFAULT_HEAD');
                        $userinfo = $userModel->getUserInfoByPhone($data_json->phone);
                        if ($userinfo) {
                            if ($userinfo['open_id'] == "") {
                                //手机号登录并且未三方登录过
                                if (!$userinfo['img']) {
                                    $userinfo['img'] = $default_head;
                                }
                                return successEncrypt($userinfo);
                            } else {
                                //第三方 登录过，用户名是手机号的用户 新开注册会员
                                goto addinfo;
                            }
                        } else {
                            //注册新手机用户
                        ...
                        $data = [
                            'username' => $data_json->phone,
                            'nickname' => $data_json->phone,
                            'phone' => $data_json->phone,
                            'img' => $default_head,
                            'money' => 68, //手机号注册默认给68书币
                            'channel' => $data_json->channel, //对应渠道
                            'channel_id' => $channelId,
                            'create_time' => time(),
                            'logintype' => 1,   //手机号登录
                        ];
                        $res = $userModel->saveInfo($data);
                        if ($res) {
                            $data = $userModel->getUserInfoByPhone($data_json->phone);
                            if (!$data['img']) {
                                $data['img'] = $default_head;
                            }
    ...
    public function index()
    {
        ...
        $this->checkVipOver($data_json);
        if (isset($data_json->id) && isset($data_json->sex)) {

            if ($data_json->id == '-1' || $data_json->id) {//$data_json->id表示用户id
                if ($data_json->sex) {
                    $res = Loader::model('Index')->banner($data_json);
                    $data = [];
                    $domain = Config::get('APP_DOMAIN');
                    foreach ($res as $k => $v) {
                        $data['listBanner'][$k]['iconUrl'] = $domain . $v['public'];
                        $data['listBanner'][$k]['id'] = $v['bookid'];
                    }
                    $date = ['b.seat' => 1, 'b.sex' => 2];//seat=1表示男女性精选sex1男2女
                    $data['listFemale'] = Loader::model('Index')->homeData($date);
                    $date = ['b.seat' => 1, 'b.sex' => 1];//seat=1表示男女性精选sex1男2女
                    $data['listMale'] = Loader::model('Index')->homeData($date);
                    $date = ['b.seat' => 2, 'b.sex' => $data_json->sex];//seat=2表示本周精选
                    $data['listWeek'] = Loader::model('Index')->homeData($date);
                    foreach ($data['listMale'] as $k => $v) {
                        $v['bookIconUrl'] = $this->get_file_path($v['bookIconUrl']);
                        $data['listMale'][$k] = $v;
                    }
                    foreach ($data['listWeek'] as $k => $v) {
                        $v['bookIconUrl'] = $this->get_file_path($v['bookIconUrl']);
                        $data['listWeek'][$k] = $v;
                    }
                    foreach ($data['listFemale'] as $k => $v) {
                        $v['bookIconUrl'] = $this->get_file_path($v['bookIconUrl']);
                        $data['listFemale'][$k] = $v;
                    }
